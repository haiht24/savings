...
class SpinRewriterWPPlugin_OptionsManager {
    ...
    public function registerSettings() {
        ...
        $optionMetaData = $this->getOptionMetaData();
	    if (is_array($optionMetaData) && count($optionMetaData) > 0) {
		    foreach ($optionMetaData as $aOptionKey => $aOptionMeta) {
    ...
    public function settingsPage() {
        ...
        ?>
        <div class="wrap">
            <h2><?php _e('System Settings', 'spin-rewriter-wp-plugin'); ?></h2>
            <table class="form-table"><tbody>
            <tr><td><?php _e('System', 'spin-rewriter-wp-plugin'); ?></td><td><?php echo php_uname(); ?></td></tr>
            <tr><td><?php _e('PHP Version', 'spin-rewriter-wp-plugin'); ?></td>
                <td><?php echo phpversion(); ?>
                <?php
                if (version_compare('5.2', phpversion()) > 0) {
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    _e('(WARNING: This plugin may not work properly with versions earlier than PHP 5.2)', 'spin-rewriter-wp-plugin');
                    echo '</span>';
                }
                ?>
                </td>
            </tr>
            <tr><td><?php _e('MySQL Version', 'spin-rewriter-wp-plugin'); ?></td>
                <td><?php echo $this->getMySqlVersion() ?>
                    <?php
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    if (version_compare('5.0', $this->getMySqlVersion()) > 0) {
                        _e('(WARNING: This plugin may not work properly with versions earlier than MySQL 5.0)', 'spin-rewriter-wp-plugin');
                    }
                    echo '</span>';
                    ?>
                </td>
            ...
            <h2><?php echo $this->getPluginDisplayName(); echo ' '; _e('Settings', 'spin-rewriter-wp-plugin'); ?></h2>
            ...
            <?php settings_fields($settingsGroup); ?>
                ...
                <p class="submit">
                    <input type="submit" class="button-primary"
                           value="<?php _e('Save Changes', 'spin-rewriter-wp-plugin') ?>"/>
                </p>
            </form>
        </div>
        <?php
    ...
    protected function getOptionValueI18nString($optionValue) {
        switch ($optionValue) {
            case 'true':
                return __('true', 'spin-rewriter-wp-plugin');
            case 'false':
                return __('false', 'spin-rewriter-wp-plugin');

            case 'Administrator':
                return __('Administrator', 'spin-rewriter-wp-plugin');
            case 'Editor':
                return __('Editor', 'spin-rewriter-wp-plugin');
            case 'Author':
                return __('Author', 'spin-rewriter-wp-plugin');
            case 'Contributor':
                return __('Contributor', 'spin-rewriter-wp-plugin');
            case 'Subscriber':
                return __('Subscriber', 'spin-rewriter-wp-plugin');
            case 'Anyone':
                return __('Anyone', 'spin-rewriter-wp-plugin');
        }
        return $optionValue;
    }

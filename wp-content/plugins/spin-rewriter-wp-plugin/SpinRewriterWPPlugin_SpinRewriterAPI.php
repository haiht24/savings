...
class SpinRewriterWPPlugin_SpinRewriterAPI {
	 ...
	 * @param $api_key
	 */
	function SpinRewriterWPPlugin_SpinRewriterAPI($username, $api_key) {
		$this->data = array();
		$this->data['wordpress_plugin'] = "true";
		$this->data['email_address'] = $username;
	...
	function setProtectedTerms($protected_terms) {
		...
		if (is_array($protected_terms) && count($protected_terms) > 0) {
			// array of words
			...
			if (is_array($protected_terms_explode) && count($protected_terms_explode) > 0) {
